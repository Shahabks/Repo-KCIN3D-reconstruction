...
for i , il in zip(rangROVER,landROVER):
    ...
    dudi.to_pickle(cathi+"/_$temp/"+ str(i)+str(i)+".pkl")

    wbdu = Workbook()
    wsdu = wbdu.active
    for r in dataframe_to_rows(dudi, index=True, header=True):
        wsdu.append(r)
    ...
    wbdo = Workbook()
    wsdo = wbdo.active
    for r in dataframe_to_rows(dodi, index=True, header=True):
        wsdu.append(r)
    for cell in wsdo['A'] + wsdo[1]:
        cell.style = 'Pandas'
    
    wbbdo = openpyxl.load_workbook(generator1, keep_vba=True)
    sheet = wbbdo['Sheet1']
    for r in dataframe_to_rows(dodi, index=True, header=True):
        sheet.append(r)
    for cell in sheet['A'] + sheet[1]:
        cell.style = 'Pandas'
    wbbdo.save(generator1)


    #moon1 = cathi + "/test/"+ str(i)+str(i)+".xlsx"
    #moon2 = cathi + "/test/"+ str(i)+".xlsx"
    #append_df_to_excel(moon1, dudi , sheet_name='Sheet', startrow=1, truncate_sheet=False)
    #append_df_to_excel(moon2, dodi , sheet_name='Sheet', startrow=1, truncate_sheet=False)
